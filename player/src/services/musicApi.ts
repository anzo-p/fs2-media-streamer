...
const fetchSampleTracks = async (): Promise<AudioTrack[]> => {
  const response = await fetchWithTimeout('https://musicbox.anzop.net/tracks/search/sample');
  const data: AudioTrack[] = await response.json();
  return data;
};

const searchTracks = async (query: AudioTrackQuery): Promise<AudioTrack[]> => {
  const response = await fetchWithTimeout('https://musicbox.anzop.net/tracks/search/query', {
    method: 'POST',
    headers: {
